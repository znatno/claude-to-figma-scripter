...
3. **Voicenter UI** has no clean per-icon SVG directory — the only artifact is an IcoMoon bundle at `/Users/ivan/ClaudeProjects/VoicenterProjects/voicenter-ui-plus/script/icons-publish/source/selection.json`. Don't try to read individual files; extend `helpers.js` instead.
